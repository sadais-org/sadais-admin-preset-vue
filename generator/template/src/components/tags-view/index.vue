...
export default {
  ...
  methods: {
        ...
        for (const tag of tags) {
          if (tag.to.path === this.$route.path) {
            ...
            if (tag.to.fullPath !== this.$route.fullPath) {
              AppModule.updateVisitedView(this.$route)
            }
            break
          }
        }
    ...
    async refreshSelectedTag(view) {
      await AppModule.delCachedView(view)
      this.$nextTick(() => {
        this.$router.replace({
          path: view.fullPath
        })
    ...
    async closeOthersTags() {
      ...
      await AppModule.delOthersViews(this.selectedTag)
      this.moveToCurrentTag()
    },
    async closeAllTags() {
      await AppModule.delAllViews()
      this.toLastView(this.visitedViews)
    },
